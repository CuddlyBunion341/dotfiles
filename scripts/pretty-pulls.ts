...
const prs: { username: string, url: string, number: number, title: string, mergedAt: string | null }[] = []
...
process.stdin.on('end', () => {
  try {
    ...
    pulls.forEach(pull => {
      ...
      prs.push({ title, number, url: html_url, username: user.login, mergedAt: merged_at })
    })
